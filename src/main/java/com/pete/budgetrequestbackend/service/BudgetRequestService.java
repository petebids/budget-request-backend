...
public class BudgetRequestService {
  ...
  @Autowired BudgetRequestRepository budgetRequestRepository;
    ...
    for (String param : requestParams.keySet()) {
      if (!allowableQueryParams.contains(param)) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST, "query param " + param + " not supported ");
      }
    }
    List<BudgetRequest> budgetRequests = new ArrayList<BudgetRequest>();
  ...
  public ResponseEntity<BudgetRequest> createBudgetRequest(CreateBudgetRequestPayload payload) {
    ...
    // these attribures are defined by bus logic , they are generated here rather than expected to
    // be part of they payload
